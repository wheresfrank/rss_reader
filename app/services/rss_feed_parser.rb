class RssFeedParser
  ...
  def article_thumbnail(item)
    smallest_image_url = nil
    smallest_area = Float::INFINITY

    item.xpath('media:group/media:content[@medium="image"]', @namespaces).each do |content|
      ...
      if area < smallest_area
        smallest_area = area
        smallest_image_url = content['url']
      end
    end

    smallest_image_url || item.at_xpath('media:thumbnail/@url', @namespaces)&.text || item.at_xpath('image')&.text || item.at_xpath('medium')&.text || item.at_xpath('thumbnail')&.text
  end
end
